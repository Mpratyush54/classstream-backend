...
// import unicode_to_krutidev_10 from './../ai_recognaition/pending_upload.js'
// const unicode_kruti = require('../ai_recognaition/pending_upload.js')
...
app.post('/', (req, res) => {
    ...
    // SELECT 
    db.query('SELECT `session` FROM `admin_data` WHERE `current`="true"', [], (err, result) => {
        if (!err) {
            var process = result
            ...
            var admission_date = dateFormat("dd/mm/yyyy")

            var rand = Math.floor(Math.random() * 999999999999);
            var yyyy = dateFormat("yyyy")
            var unique_identification_no = rand.toString() + yyyy.toString()
            db.query('SELECT MAX(Admission_no) FROM user_data', [], (err, result2) => {
                if (!err) {

                    var Admission_no = result2[0]['MAX(Admission_no)'] + 1

                    // For Photo
                    const base64StringStudent = data.croppedImage;

                    // Remove the data:image/jpeg;base64, part from the beginning of the string
                    const base64Image = base64StringStudent.split(';base64,').pop();

                    // Convert base64 string to Buffer
                    const imageBuffer = Buffer.from(base64Image, 'base64');

                    // Save the Buffer to a file
                    const path_base64StringStudent = `./student_photo/${unique_identification_no}.jpeg`
                    fs.writeFile(path_base64StringStudent, imageBuffer, (error) => {
                        if (error) {
                            console.error(error);
                        } else {
                    ...
                    const base64StringStudentSig = data.croppedImageSig;

                    // Remove the data:image/jpeg;base64, part from the beginning of the string
                    const base64ImageSig = base64StringStudentSig.split(';base64,').pop();

                    // Convert base64 string to Buffer
                    const imageBufferSig = Buffer.from(base64ImageSig, 'base64');

                    // Save the Buffer to a file
                    const path_base64StringStudentSig = `./Student _sig/${unique_identification_no}.jpeg`
                    fs.writeFile(path_base64StringStudentSig, imageBufferSig, (error) => {
                        if (error) {
                            console.error(error);
                        } else {
                            console.log('Image saved successfully');
                    ...
                    // For Adhar front
                    const base64Stringadharfront = data.croppedImageAdhar;

                    // Remove the data:image/jpeg;base64, part from the beginning of the string
                    const base64Imageadharfront = base64Stringadharfront.split(';base64,').pop();

                    // Convert base64 string to Buffer
                    const imageBufferadharfront = Buffer.from(base64Imageadharfront, 'base64');

                    // Save the Buffer to a file
                    const path_base64Stringadharfront = `./student_adhar_front/${unique_identification_no}.jpeg`
                    fs.writeFile(path_base64Stringadharfront, imageBufferadharfront, (error) => {
                        if (error) {
                            console.error(error);
                        } else {
                            console.log('Image saved successfully');
                    ...
                    // For Adhar back
                    const base64Stringadharback = data.croppedImageAdhar2;

                    // Remove the data:image/jpeg;base64, part from the beginning of the string
                    const base64Imageadharback = base64Stringadharback.split(';base64,').pop();

                    // Convert base64 string to Buffer
                    const imageBufferaadharback = Buffer.from(base64Imageadharback, 'base64');

                    // Save the Buffer to a file
                    const path_base64Stringadharback = `./student_adhar_back/${unique_identification_no}.jpeg`
                    fs.writeFile(path_base64Stringadharback, imageBufferaadharback, (error) => {
                        if (error) {
                            console.error(error);
                        } else {
                            console.log('Image saved successfully');
                    ...
                    const unno = unique_identification_no
                   
                    db.query(' select * from processing_data Where id = ?',
                        [data.id, ], (err, result) => {
                            if (!err) {
                                var process = result[0]
                                    console.log(process);
                                    const id = process.id
                                    const image_name = process.image_name
                                    const jsondata = process.jsondata
                                    // res.send({ status: true, error: false, mes: [id , image_name , jsondata] })
            
            
                                if(data.isMobile){
                                    data.Address_in_hindi =    unicode_to_krutidev_10(data.Address_in_hindi)
                                    data.Father_Name_hindi =    unicode_to_krutidev_10(data.Father_Name_hindi)
                                    data.Mother_Name_hindi =    unicode_to_krutidev_10(data.Mother_Name_hindi)
                                    data.Name_hindi =    unicode_to_krutidev_10(data.Name_hindi)
                                }
                                
                                db.query('INSERT INTO `user_data`(`Address`,    `Address_in_hindi`,      `AdharNo`,     `Date_of_birth`,       `Father_Name_english`,     `Father_Name_hindi`,      `MinorityGroups`,      `MobileNo`,    `MotherTongue`,         `Mother_Name_english`,      `Mother_Name_hindi`,       `Name_english`,       `Name_hindi`,      `Pincode`,      `SelectCategory`,      `gender`,       `croppedImage`,           `croppedImageAdhar`,          `croppedImageAdhar2`,        `admission_date`,     `By_user`,                `Admission_no`,          `Session`,      `unique_identification_no`, `croppedImageSig`,           `class` , `id` ,`image_name` , `jsondata`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                                    [                             data.Address, data.Address_in_hindi,    data.AdharNo, data.Date_of_birth,      data.Father_Name_english, data.Father_Name_hindi,  data.MinorityGroups,    data.MobileNo, data.MotherTongue,       data.Mother_Name_english,  data.Mother_Name_hindi,    data.Name_english,    data.Name_hindi,   data.Pincode, data.SelectCategory,     data.gender,    path_base64StringStudent, path_base64Stringadharfront,   path_base64Stringadharback, admission_date,      req.body.student_username, Admission_no,             Session,        unno,                      path_base64StringStudentSig, data.class,id  ,image_name , jsondata], (err, result) => {
                                        if (!err) {
                                            var process = result
            
                                            db.query('Delete from processing_data Where id = ?',
                                                [data.id], (err, result) => {
                                                    if (!err) {
                                                        var process = result
                        
                                                        res.send({ status: true, error: false, mes: 'Student Registered' })
                        
                                                    } else {
                                                        if (err.sqlState = "23000") {
                                                            console.log(err);
                        
                                                            res.send({ status: true, error: true, mes: 'Adhar No Already registered' })
                                                        }
        ...
        if (!err) {
            ...
            db.query('SELECT MAX(Admission_no) FROM user_data', [], (err, result2) => {
                if (!err) {
                    ...
                    db.query('INSERT INTO `user_data`(`Address`,    `Address_in_hindi`,      `AdharNo`,   `Date_of_birth`,       `Father_Name_english`,     `Father_Name_hindi`,  `MinorityGroups`,      `MobileNo`,    `MotherTongue`,     `Mother_Name_english`,      `Mother_Name_hindi`,       `Name_english`,       `Name_hindi`,      `Pincode`,      `SelectCategory`,      `gender`,       `croppedImage`,           `croppedImageAdhar`,          `croppedImageAdhar2`,        `admission_date`,     `By_user`,              `Admission_no`,          `Session`,      `unique_identification_no`, `croppedImageSig`, `class`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                        [data.Address, data.Address_in_hindi, data.AdharNo, data.Date_of_birth, data.Father_Name_english, data.Father_Name_hindi, data.MinorityGroups, data.MobileNo, data.MotherTongue, data.Mother_Name_english, data.Mother_Name_hindi, data.Name_english, data.Name_hindi, data.Pincode, data.SelectCategory, data.gender, path_base64StringStudent, path_base64Stringadharfront, path_base64Stringadharback, admission_date, req.body.student_username, Admission_no, Session, unno,path_base64StringStudentSig, data.class], (err, result) => {
                            if (!err) {
                                var process = result
...
app.post('/fetch', (req, res) => {
    ...
    db.query('SELECT  `Name_english`, `unique_identification_no` FROM `user_data` WHERE `By_user` =?', [req.body.student_username], (err, result) => {
        var process = result[0]
        if (!err) {
...
app.post('/edit-data', (req, res) => {
    ...
    db.query('SELECT *  FROM `user_data` WHERE `unique_identification_no` =?', [req.body.unique_identification_no], (err, result) => {
        ...
        if (!err) {
            ...
            data['uni_Address_in_hindi'] = kruti_to_unicode(data.Address_in_hindi)
            data['uni_Father_Name_hindi'] = kruti_to_unicode(data.Father_Name_hindi)
            data['uni_Name_hindi'] = kruti_to_unicode(data.Name_hindi)
            data['uni_Mother_Name_hindi'] = kruti_to_unicode(data.Mother_Name_hindi)
            console.log(data);
            
            res.send({ status: true, error: false, data: data })
...
app.post('/delete-data', (req, res) => {
    ...
    db.query('DELETE FROM `user_data` WHERE `unique_identification_no` =?', [req.body.unique_identification_no], (err, result) => {
        var process = result[0]
        if (!err) {
...
app.post('/search-data', (req, res) => {
    ...
    db.query('SELECT * FROM `user_data` WHERE `Address` LIKE %?%  OR `Address_in_hindi` LIKE %?% OR`AdharNo`LIKE %?% OR`Date_of_birth`LIKE %?% OR`Father_Name_english`LIKE %?% OR`Father_Name_hindi`LIKE %?% OR`MinorityGroups`LIKE %?% OR`MobileNo`LIKE %?% OR`MotherTongue`LIKE %?% OR`Mother_Name_english`LIKE %?% OR`Mother_Name_hindi`LIKE %?% OR`Name_english`LIKE %?% OR`Name_hindi`LIKE %?% OR`Pincode`LIKE %?% OR`SelectCategory`LIKE %?% OR`gender`LIKE %?% OR`croppedImage`LIKE %?% OR`croppedImageAdhar`LIKE %?% OR`croppedImageAdhar2`LIKE %?% OR`sr_no`LIKE %?% OR`admission_date`LIKE %?% OR`By_user`LIKE %?% OR`Admission_no`LIKE %?% OR`Roll_no`LIKE %?% OR`Session`LIKE %?% OR`unique_identification_no`LIKE %?% OR`croppedImageSig`LIKE %?% OR`class`LIKE %?% OR', [req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,req.body.serarch_data,], (err, result) => {
        var process = result[0]
        if (!err) {
...
app.post('/update-edit-data', (req, res) => {
    ...
    const id = req.body.id
    const Class = req.body.class
    const unique_identification_no = req.body.unique_identification_no
    // SELECT 
    const path_base64StringStudent = `./student_photo/${unique_identification_no}.jpeg`
    const path_base64StringStudentSig = `./Student_sig/${unique_identification_no}.jpeg`
    const path_base64Stringadharfront = `./student_adhar_front/${unique_identification_no}.jpeg`
    const path_base64Stringadharback = `./student_adhar_back/${unique_identification_no}.jpeg`
    data['Address_in_hindi'] = unicode_to_krutidev_10(data.Address_in_hindi)
    data['Father_Name_hindi'] = unicode_to_krutidev_10(data.Father_Name_hindi)
...
console.log(req.body.unique_identification_no);
console.log(data.class);
    Promise.all([
        saveImage(req.body.croppedImage, path_base64StringStudent),
        saveImage(req.body.croppedImageSig, path_base64StringStudentSig),
        saveImage(req.body.croppedImageAdhar, path_base64Stringadharfront),
        saveImage(req.body.croppedImageAdhar2, path_base64Stringadharback)
    ]).then(() => {

    
      // Log the class to ensure it's correct
...
db.query(`
    ...
    SET 
        ...
        croppedImageSig = ?, 
        class = ? 
    WHERE unique_identification_no = ?
`, [
    data.Address, 
    data.Address_in_hindi, 
    data.AdharNo, 
    data.Date_of_birth, 
    ...
    data.Mother_Name_hindi, 
    data.Name_english, 
    data.Name_hindi, 
    data.Pincode, 
    data.SelectCategory, 
    data.gender, 
    path_base64StringStudent, 
    path_base64Stringadharfront, 
    path_base64Stringadharback, 
    path_base64StringStudentSig, 
    data.class, 
    unique_identification_no
], (err, result) => {
    if (err) {
        console.log('Error during database update:', err);
        res.send({ status: false, error: true, mes: 'Something went wrong during update' });
    } else {
        console.log('Update successful:', result);
        res.send({ status: true, error: false, mes: 'Student Update', data: result });
    }
});
...
//                     db.query('UPDATE `user_data_` SET `Address`=?,      `Address_in_hindi`=?,   `AdharNo`=?,    `Date_of_birth`=?,       `Father_Name_english`=?,    `Father_Name_hindi`=?,`    MinorityGroups`=?,      `MobileNo`=?,        `MotherTongue`=?,          `Mother_Name_english`=?,        `Mother_Name_hindi`=?,   `Name_english`=? ,     `Name_hindi`=?,  `Pincode`=?,       `SelectCategory`=?,  `gender`=?,         `croppedImage`=?,          `croppedImageAdhar`=?,       `croppedImageAdhar2`=?,                    `croppedImageSig`=?,        `class`=? WHERE `unique_identification_no` = ?',
//                                             [           data.Address,    data.Address_in_hindi,  data.AdharNo,   data.Date_of_birth,       data.Father_Name_english,   data.Father_Name_hindi,  data.MinorityGroups,        data.MobileNo,       data.MotherTongue,         data.Mother_Name_english,     data.Mother_Name_hindi,   data.Name_english, data.Name_hindi,      data.Pincode,  data.SelectCategory,     data.gender,   path_base64StringStudent,   path_base64Stringadharfront, path_base64Stringadharback,             , path_base64StringStudentSig,  data.class ,     unique_identification_no], (err, result) => {
//                             if (!err) {
//                                 var process = result
// console.log(result , data);

//                                 res.send({ status: true, error: false, mes: 'Student Update' ,data })
...
const saveImage = (base64String, path) => {
...
function kruti_to_unicode(legacy_text_var = "", max_text_size = 6000) {
  ...
  module.exports = { unicode_to_krutidev_10,kruti_to_unicode };
